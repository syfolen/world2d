...
module world2d {
    ...
    export class Physics2D {
        ...
        static raycast(origin: IVector2D, direction: IVector2D, maxDistance: number, layers: CollisionLayerEnum = 0, type: RaycastTypeEnum = RaycastTypeEnum.CLOSEST): IRaycastResult[] {
            ...
            bounds.updateBounds(
                suncom.Mathf.min(origin.x, destination.x),
                suncom.Mathf.max(origin.x, destination.x),
                suncom.Mathf.min(origin.y, destination.y),
                suncom.Mathf.max(origin.y, destination.y)
            );
            ...
            for (let i: number = 0; i < transforms.length; i++) {
                ...
                if (type === RaycastTypeEnum.CLOSEST) {
                    ...
                    array[0] = out;
                    bounds.updateBounds(
                        suncom.Mathf.min(origin.x, out.p1.x),
                        suncom.Mathf.max(origin.x, out.p1.x),
                        suncom.Mathf.min(origin.y, out.p1.y),
                        suncom.Mathf.max(origin.y, out.p1.y)
                    );
                    // DrawAPI2D.drawRect(bounds.left, bounds.top, bounds.right - bounds.left, bounds.bottom - bounds.top, "#FF0000");
                }
                else {
